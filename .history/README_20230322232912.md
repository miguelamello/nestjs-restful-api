...
Obs: 1) Você pode rodar os testes individuamente executando `npx jest target-test-file.ts`. Aonde `target-test-file.ts` é qualquer dos arquivos de testes dentro do diretório `test` directory. 2) You can look for the file `manitest.txt` inside `__tests__` for a quick introduction for all the Integration Tests available.
