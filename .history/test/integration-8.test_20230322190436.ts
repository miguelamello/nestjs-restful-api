...
/**
 * Let's test the inexistent endpoint.
 */
describe('GET /xxx', () => {

  const endpoint = `${env.protocol}://${env.host}:${env.port}/xxx`;
  ...
  it(
    ...
    async () => {
      ...
      //responseBody.message must be "Cannot GET /convert/xxx"
      expect(responseBody.message).toBe('Cannot GET /convert/xxx');
    }
  );
