...
Obs: 1) Você pode rodar os testes individuamente executando `npx jest target-test-file.ts`. Where `target-test-file.ts` right name you can find inside `__tests__` directory. 2) You can look for the file `manitest.txt` inside `__tests__` for a quick introduction for all the Integration Tests available.
